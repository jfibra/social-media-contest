...
export default function AdminSubmissionsPage() {
  ...
  // Function to fetch contest with submissions
  const fetchContestWithSubmissions = async () => {
    try {
      ...
      if (data.submissions) {
        ...
        applyFilters(data.submissions, statusFilter)
  ...
  const applyFilters = (submissionsToFilter: Submission[], status: string) => {
    let filtered = [...submissionsToFilter]
  ...
  // Update filtered submissions when filter changes
  useEffect(() => {
    applyFilters(submissions, statusFilter)
  ...
  const exportToCSV = () => {
    try {
      ...
      let csvContent = "ID,Name,Email,Phone,FB Post Link,Initial Likes,Verified Likes,Status,Submission Date\n"

      // Use filtered submissions if not showing ranking, otherwise use ranked submissions
      const dataToExport = showRanking ? rankedSubmissions : filteredSubmissions
      ...
      dataToExport.forEach((sub) => {
        ...
        const verifiedLikes = sub.verified_likes || sub.likes_verification_json?.likes || ""
        const row = [
          sub.id,
          escapeCSV(sub.name),
          escapeCSV(sub.email),
          escapeCSV(sub.phone),
          escapeCSV(sub.fb_post_link),
          sub.initial_likes,
          verifiedLikes,
          escapeCSV(sub.status),
          escapeCSV(sub.submission_time),
        ]
        csvContent += row.join(",") + "\n"
      })
      ...
      link.setAttribute("download", `contest-${contestId}-submissions-${new Date().toISOString().slice(0, 10)}.csv`)
      link.style.visibility = "hidden"
      document.body.appendChild(link)
      link.click()
      document.body.removeChild(link)

      showSuccessAlert("CSV exported successfully!")
    } catch (error) {
      console.error("Error exporting CSV:", error)
      showErrorAlert(`Failed to export CSV: ${error instanceof Error ? error.message : String(error)}`)
    }
  }
  ...
  return (
    <div className="min-h-screen pt-24 pb-16">
      <div className="container mx-auto px-4">
        <div className="flex justify-between items-center mb-8">
          <div>
            <div className="flex items-center gap-2 mb-2">
              <Link href="/admin/contests" className="text-realty-primary hover:text-realty-secondary">
        ...
        {/* Filter controls - only show when not in ranking view */}
        {!showRanking && (
          <div className="mb-6 p-4 bg-gray-50 rounded-lg shadow-sm">
            <div className="flex items-center gap-4">
              ...
              <select
                ...
                className="px-3 py-1 border border-gray-300 rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-realty-primary"
              >
        ...
        ) : !showRanking && filteredSubmissions.length > 0 ? (
          <div className="bg-white rounded-lg shadow-md overflow-hidden">
            <div className="overflow-x-auto">
              <table className="min-w-full divide-y divide-gray-200">
                <thead className="bg-gray-50">
